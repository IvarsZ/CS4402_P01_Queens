import subprocess
import glob

min = 2
max = 6
few_times = 1
many_times = 1
parts = ['p1', 'p2', 'p4']

for m in range(min, max + 1):

  times = 0
  if m <= 7:
    times = many_times
  else:
    times = few_times

  results_file = open("results.txt", "a")

  for part in parts:
    
    input_file = 'src/m_queens_{0}.eprime'.format(part)
    
    number_of_queens = -1

    for i in range(times):

      # Run savilerow
      output_file = 'solutions/{0}_queens_{1}_{2}.solution'.format(m, part, i)
      args = './savilerow -in-eprime {0} -run-minion minion -out-solution {1}'.format(input_file, output_file).split() + ['-params', 'letting n be {0}'.format(m)]
      subprocess.call(args)

      # Read/write output
      nodes = -1
      time = -1
      with open(output_file) as fp:
        for line in fp:
          words = line.split()
          if len(words) >= 4:
            if words[1] == 'numberOfQueens':
              number_of_queens = words[3]
            elif words[2] == 'TotalTime:':
              time = words[3]
            elif words[2] == 'Nodes:':
              nodes = words[3]
      #results_file.write(part + ' ' + m + ' ' + i + ' ' + time + ' ' + nodes + ' ' + number_of_queens)
      results_file.write(' '.join(str(i) for i in [part, m, i, time, nodes, number_of_queens]) + '\n')
  
    
    # Find the number of solutions
    input_file = 'src/m_queens_{0}_all.eprime'.format(part)
    output_file = 'solutions/{0}_queens_{1}_all.solution'.format(m, part)
    args = './savilerow -in-eprime {0} -run-minion minion -out-solution {1} -all-solutions'.format(input_file, output_file).split() + ['-params', 'letting n be {0}'.format(m)]
    subprocess.call(args)

    # by counting files with minimal number of queens used
    print output_file + "*"
    print number_of_queens

    number_of_solutions = 0
    solutions = glob.glob(output_file + "*")
    for solution in solutions: 
      with open(solution) as fp:
        for line in fp:
          words = line.split()
          if words[1] == 'numberOfQueens' and words[3] == number_of_queens:
            number_of_solutions += 1

    queens_file = open("number_of_solutions.txt", "a")
    queens_file.write(' '.join(str(i) for i in [part, m, number_of_solutions]) + '\n')
    queens_file.close()
 
  results_file.close()
